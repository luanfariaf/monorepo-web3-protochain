...
export default class Blockchain {
  ...
  constructor() {
    ...
    this.blocks = [new Block({
      index: this.nextIndex, transactions: [new Transaction({
        type: TransactionType.FEE,
        data: new Date().toString(),
      } as Transaction)] as Transaction[] } as Block)]
    this.nextIndex++
  }
  ...
  addTransaction(transaction: Transaction): Validation {
    ...
    if (this.blocks.some(b => b.transactions.some(tx => tx.hash === transaction.hash))) return new Validation(false, 'Duplicated TX in blockchain')

    if (this.mempool.some(tx => tx.hash === transaction.hash)) return new Validation(false, 'Duplicated tx in mempool')
